-- PARTIE 3 : Reviews, Favoris, Jours de circuit, Contributions, Scores

-- 6. Reviews (avis) sur les offres et circuits
INSERT INTO reviews (id, author_id, target_type, target_id, rating, comment, created_at, updated_at)
SELECT gen_random_uuid(), '7b83e87d-276d-4d89-bb00-ab8ea1243a14'::uuid, 'offer', o.id,
...
INSERT INTO reviews (id, author_id, target_type, target_id, rating, comment, created_at, updated_at)
SELECT gen_random_uuid(), 'a602737a-b07d-4a41-b9c3-cdf1be17036a'::uuid, 'offer', o.id,
  4 + floor(random() * 2)::int,
  (ARRAY['Très belle expérience, cadre magnifique.','Bon rapport qualité-prix, à refaire.','Activité intéressante mais prévoir plus de temps.','Excellente organisation, tout était parfait.'])[floor(random() * 4 + 1)::int],
...
INSERT INTO reviews (id, author_id, target_type, target_id, rating, comment, created_at, updated_at)
SELECT gen_random_uuid(), 'b09808ee-30a9-4089-bbf7-698e73004ef4'::uuid, 'circuit', c.id,
  4 + floor(random() * 2)::int,
  (ARRAY['Circuit magnifique, paysages variés et guide excellent.','Très beau voyage, organisation parfaite.','À faire absolument, une expérience unique en Tunisie.'])[floor(random() * 3 + 1)::int],
...
INSERT INTO reviews (id, author_id, target_type, target_id, rating, comment, created_at, updated_at)
SELECT gen_random_uuid(), '90b4c5bf-4a47-4737-b033-f7385e22a2e6'::uuid, 'circuit', c.id,
  4 + floor(random() * 2)::int,
  (ARRAY['Superbe circuit, bien organisé et guide sympathique.','Dépaysement total, je recommande les yeux fermés.','Une aventure humaine et naturelle exceptionnelle.'])[floor(random() * 3 + 1)::int],
  NOW() - (random() * interval '18 days'), NOW()
FROM circuits c
WHERE c.status = 'approved'
LIMIT 5;
...
FROM (VALUES
  ...
  ('b09808ee-30a9-4089-bbf7-698e73004ef4'::uuid),
  ('90b4c5bf-4a47-4737-b033-f7385e22a2e6'::uuid)
) AS u(user_id)
CROSS JOIN (SELECT id FROM offers WHERE status = 'approved' ORDER BY random() LIMIT 3) AS o
...
CROSS JOIN (SELECT id FROM circuits WHERE status = 'approved' ORDER BY random() LIMIT 2) AS c
ON CONFLICT (user_id, target_type, target_id) DO NOTHING;

-- 8. Jours de circuit avec programme
DO $$
DECLARE
  circ RECORD;
  day_id UUID;
...
BEGIN
  FOR circ IN SELECT id, title, duration_days, start_date FROM circuits WHERE status = 'approved' AND duration_days IS NOT NULL LOOP
    FOR i IN 1..circ.duration_days LOOP
      ...
      VALUES (
        ...
        CASE
          ...
          WHEN circ.title ILIKE '%djerba%' THEN
            ...
            END
          WHEN circ.title ILIKE '%kroumirie%' OR circ.title ILIKE '%ain draham%' OR circ.title ILIKE '%ain draham%' THEN
            CASE i
      ...
      -- Programme items pour chaque jour
      ...
      VALUES
        (gen_random_uuid(), day_id,
         ...
         CASE i WHEN 1 THEN '09:00' WHEN circ.duration_days THEN '08:00' ELSE '08:00' END,
         CASE i WHEN 1 THEN '10:00' WHEN circ.duration_days THEN '09:30' ELSE '09:00' END,
         true, true, NOW());
...
-- 10. Contributions sur les lieux + votes
...
SELECT gen_random_uuid(), p.id::uuid, u.user_id::uuid, u.role,
  (ARRAY['description', 'description', 'images'])[floor(random() * 3 + 1)::int],
  (ARRAY[
...
-- 11. Recalcul des popularity_scores
