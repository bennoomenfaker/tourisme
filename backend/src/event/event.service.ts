...
export class EventService {
  ...
  async findUpcomingByPlace(placeId: string): Promise<Event[]> {
    ...
    return this.repo.find({
      where: { place_id: placeId, status: 'published' },
      order: { start_date: 'ASC' },
    }).then((events) => events.filter((e) => new Date(e.start_date) >= now || (e.end_date && new Date(e.end_date) >= now)));
  }

  async create(placeId: string, userId: string, dto: CreateEventDto): Promise<Event> {
  ...
  async update(eventId: string, userId: string, dto: UpdateEventDto): Promise<Event> {
    ...
    if (event.created_by !== userId) throw new ForbiddenException('Accès refusé.');
    if (dto.title !== undefined) event.title = dto.title;
    if (dto.description !== undefined) event.description = dto.description;
    if (dto.event_type !== undefined) event.event_type = dto.event_type;
    if (dto.start_date !== undefined) event.start_date = new Date(dto.start_date);
    if (dto.end_date !== undefined) event.end_date = dto.end_date ? new Date(dto.end_date) : null;
    if (dto.images !== undefined) event.images = dto.images.length ? dto.images : null;
