...
export class ReportsService {
  ...
  private async getUserInfo(userId: string, role: string) {
    ...
    return { user_id: userId, full_name: entity?.full_name ?? null, photo: entity?.photo ?? null, role: r, email: user?.email ?? null, status: user?.status ?? null };
  }
  ...
  async resolveReport(reportId: string, action: string, adminNote: string, banDays?: number) {
    ...
    if (reporterUser) {
      const reporterInfo = await this.getUserInfo(report.reporter_id, report.reporter_role);
