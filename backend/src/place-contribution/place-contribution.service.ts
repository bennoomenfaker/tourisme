...
export class PlaceContributionService {
  ...
  async create(
    ...
    const pub = await this.pubRepo.findOne({
      where: { id: publicationId, type: 'place', status: 'approved' },
    });
    if (!pub) throw new NotFoundException('Lieu introuvable ou non approuvé.');

    if (dto.type === 'description' && !dto.content?.trim()) {
      throw new BadRequestException(
        'Le contenu est requis pour une contribution de type description.',
      );
    }
  ...
  async findByPublication(publicationId: string, viewerId?: string) {
    ...
    const uniqueAuthors = [
      ...new Set(contributions.map((c) => `${c.user_id}:${c.user_role}`)),
    ];
    ...
    const allVotes = await this.voteRepo.find({
      where: { contribution_id: In(contribIds) },
    });
    ...
    return contributions.map((c) => {
      ...
      let userVoted = false;
      let imageVotes:
        | Record<number, { vote_count: number; user_voted: boolean }>
        | undefined;
      ...
      if (c.type === 'images' && c.images) {
        ...
        for (let i = 0; i < c.images.length; i++) {
          ...
          imageVotes[i] = {
            ...
            user_voted: viewerId
              ? votesForImg.some((v) => v.user_id === viewerId)
              : false,
          };
        }
        userVoted = viewerId
          ? contribVotes.some((v) => v.user_id === viewerId)
          : false;
      } else {
        // Description: votes have image_index = null
        const descVotes = contribVotes.filter((v) => v.image_index === null);
        userVoted = viewerId
          ? descVotes.some((v) => v.user_id === viewerId)
          : false;
      }
      ...
      return {
        ...
        author: authorMap.get(`${c.user_id}:${c.user_role}`) ?? {
          user_id: c.user_id,
  ...
  async toggleVote(
    ...
    const imgIdx =
      imageIndex !== undefined && imageIndex >= 0 ? imageIndex : null;
    ...
    return {
      voted,
      image_index: imgIdx,
      vote_count: contrib.vote_count,
      image_vote_count: imageVoteCount,
    };
  }
