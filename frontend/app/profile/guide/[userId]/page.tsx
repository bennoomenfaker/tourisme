...
import {
  ...
  Clock, Leaf, MoreVertical, Flag, X, Check, ChevronLeft, ChevronRight, Users, ShieldCheck, ShieldBan, Send,
} from "lucide-react";
...
function OfferRow({ offer, onClick }: { offer: Offer; onClick: () => void }) {
  ...
  return (
    <div onClick={onClick} className="flex gap-4 p-4 hover:bg-slate-50 rounded-2xl transition-colors cursor-pointer">
      <div className={`w-20 h-20 rounded-xl overflow-hidden shrink-0 flex items-center justify-center ${!img ? "bg-gradient-to-br from-emerald-400 to-teal-300" : ""}`}>
        {img ? <img src={img} alt={offer.title} className="w-full h-full object-cover" /> : <span className="material-symbols-outlined text-white/70" style={{ fontSize: 32 }}>hiking</span>}
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          {offer.offer_type && (
            <span className="text-[10px] font-black uppercase tracking-widest px-2.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700">
