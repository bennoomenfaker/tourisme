...
import {
  ...
  X, ChevronLeft, ChevronRight, Users, Tag, ShieldBan, Send,
} from "lucide-react";
...
function ProjectCard({ project, onClick }: { project: Project; onClick: () => void }) {
  const img = project.photos?.[0] ?? project.photo;
  return (
    <div onClick={onClick} className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden cursor-pointer hover:shadow-md transition-shadow">
      <div className={`relative h-36 ${!img ? "bg-gradient-to-br from-blue-400 to-cyan-300" : ""}`}>
        {img ? <img src={img} alt={project.name} className="w-full h-full object-cover" /> : <span className="material-symbols-outlined absolute inset-0 flex items-center justify-center text-white/30" style={{ fontSize: 56 }}>domain</span>}
        <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent" />
        {project.region && (
          <span className="absolute bottom-2 left-2.5 flex items-center gap-1 text-white text-[11px] font-semibold bg-black/40 backdrop-blur-sm px-2.5 py-1 rounded-full">
            <MapPin size={10} />{project.region}
          </span>
        )}
      </div>
      <div className="p-4">
        <h4 className="font-extrabold text-slate-800 text-sm line-clamp-1 mb-1">{project.name}</h4>
        {project.project_type && project.project_type.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-2">
            {project.project_type.slice(0, 2).map((t) => <span key={t} className="text-[10px] font-bold text-primary bg-primary/10 px-2 py-0.5 rounded-full capitalize">{t}</span>)}
          </div>
        )}
        {project.eco_labels && project.eco_labels.length > 0 && (
          <span className="flex items-center gap-1 text-[10px] font-bold text-primary"><Leaf size={10} />{project.eco_labels[0]}</span>
        )}
      </div>
    </div>
  );
...
function OfferRow({ offer, onClick }: { offer: Offer; onClick: () => void }) {
  ...
  return (
    <div onClick={onClick} className="flex gap-4 p-4 hover:bg-slate-50 rounded-2xl transition-colors cursor-pointer">
      <div className={`w-20 h-20 rounded-xl overflow-hidden shrink-0 flex items-center justify-center ${!img ? "bg-gradient-to-br from-blue-400 to-cyan-300" : ""}`}>
        {img ? <img src={img} alt={offer.title} className="w-full h-full object-cover" /> : <span className="material-symbols-outlined text-white/70" style={{ fontSize: 32 }}>storefront</span>}
      </div>
      <div className="flex-1 min-w-0">
        {offer.offer_type && (
          <span className="text-[10px] font-black uppercase tracking-widest px-2.5 py-0.5 rounded-full bg-blue-50 text-blue-700 mb-1 inline-block">
            {OFFER_TYPE_LABELS[offer.offer_type] ?? offer.offer_type}
          </span>
        )}
        <h4 className="font-extrabold text-slate-800 text-sm line-clamp-1 mb-1">{offer.title}</h4>
        <div className="flex items-center gap-3 text-xs text-slate-400 font-medium flex-wrap">
          {offer.region && <span className="flex items-center gap-1"><MapPin size={10} className="text-primary" />{offer.region}</span>}
          {offer.duration && <span className="flex items-center gap-1"><Clock size={10} />{offer.duration}</span>}
          {offer.price !== null && <span className="font-black text-primary">{offer.price} DT</span>}
        </div>
      </div>
    </div>
  );
                  ...
                  : <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {profile.projects.map((p) => (
                        <div key={p.id} ref={(el) => { projectRefs.current[p.id] = el; }}
                          className={`bg-white rounded-2xl border shadow-sm overflow-hidden ${highlightedProjectId === p.id ? "border-primary ring-2 ring-primary ring-offset-2 bg-primary/5" : "border-slate-100"}`}>
                  ...
                  : <div className="divide-y divide-slate-50">
                      {profile.offers.map((o) => (
                        <div key={o.id} ref={(el) => { offerRefs.current[o.id] = el; }}
                          className={highlightedOfferId === o.id ? "ring-2 ring-primary ring-offset-2 bg-primary/5 rounded-2xl" : ""}>
                          <OfferRow offer={o} onClick={() => { setSelectedOffer(o); setSliderIdx(0); }} />
                          <PubInteractions
                            pubId={o.id}
                            token={token}
                            viewerId={viewerId}
